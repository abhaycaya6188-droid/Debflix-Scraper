...
function buildMovieMap() {
    ...
    console.log(
        `[CTG-3] Movie map built (${movieMap.size} titles)`
    );
...
function buildTvMap() {
    ...
    console.log(
        `[CTG-3] TV map built (${tvMap.size} episodes)`
    );
...
function loadIndex() {
    ...
    console.log(
        `[CTG-3] Loaded ${index.length} entries`
    );
...
function search(query) {
    ...
    if (process.env.CTG_DEBUG === "true") {
        ...
        console.log(
            `[CTG-3] "${query.title}" -> ${results.length} results in ${Date.now() - start} ms`
        );
