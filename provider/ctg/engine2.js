...
function buildMovieMap() {
    ...
    console.log(
        `[CTG-2] Movie map built (${movieMap.size} titles)`
    );
...
function buildTvMap() {
    ...
    console.log(
        `[CTG-2] TV map built (${tvMap.size} episodes)`
    );
...
function loadIndex() {
    ...
    console.log(
        `[CTG-2] Loaded ${index.length} entries`
    );
...
function search(query) {
    ...
    if (process.env.CTG_DEBUG === "true") {
        ...
        console.log(
            `[CTG-2] "${query.title}" -> ${results.length} results in ${Date.now() - start} ms`
        );
