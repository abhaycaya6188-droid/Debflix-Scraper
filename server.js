...
if (pathname === "/api/dahmer-remux") {
  const sourceUrl = url.parse(req.url, true).query.url;
  ...
  try {
    ...
    res.statusCode = 400;
    return res.end("Invalid source URL");
  }

  if (
    parsedSource.protocol !== "https:" ||
    parsedSource.hostname !== "a.111477.xyz" ||
    !/\.mkv$/i.test(parsedSource.pathname)
  ) {
    res.statusCode = 403;
    return res.end("Source is not allowed");
  }
  ...
  ffmpeg.once("error", error => {
    console.error("DAHMER REMUX START FAILED", error);
    if (!res.headersSent) {
      res.statusCode = 503;
      res.end("Provider 5 remux is unavailable");
    } else {
      res.destroy(error);
    }
  });
  ...
  ffmpeg.once("close", code => {
    if (code && code !== 0) {
      console.error("DAHMER REMUX FAILED", code, stderr);
    }
